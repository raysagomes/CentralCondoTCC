...
export default function Payments() {
  ...
  const { payments: allPayments, loading: paymentsLoading, markAsPaid, refetch } = usePayments();
  const [activeTab, setActiveTab] = useState('pending');
  const [showBoletoModal, setShowBoletoModal] = useState(false);
  const [showAddModal, setShowAddModal] = useState(false);
  const [selectedPayment, setSelectedPayment] = useState<any>(null);
  const [newPayment, setNewPayment] = useState({ title: '', description: '', amount: '', dueDate: '' });

  const payments = {
    pending: allPayments.filter(p => !p.paid && new Date(p.dueDate) >= new Date()),
    paid: allPayments.filter(p => p.paid),
    overdue: allPayments.filter(p => !p.paid && new Date(p.dueDate) < new Date())
  };
  ...
  const getTypeColor = (type) => {
    switch (type) {
      case 'condominium': return 'bg-blue-500/20 text-blue-400';
      ...
      case 'fine': return 'bg-red-500/20 text-red-400';
            ...
            {activeTab === 'pending' && (
              <div className="space-y-4">
                {payments.pending.map((payment) => (
                  <div key={payment.id} className={`${theme.secondaryBg} border ${theme.border} rounded-lg p-4 hover:border-blue-500/50 transition-all duration-200`}>
                  ...
                  <div key={payment.id} className="bg-green-500/10 border border-green-500/30 rounded-lg p-4">
                    <div className="flex justify-between items-start">
                      ...
                      <div className="text-right">
                        ...
                        <div className="flex space-x-2 mt-2">
                          <button className={`${theme.secondaryBg} ${theme.textSecondary} px-3 py-1 rounded-lg ${theme.hover} transition-colors text-sm`}>
                            Ver Comprovante
                          </button>
                          <button 
                            onClick={() => handleDeletePayment(payment.id)}
            ...
            <div className="bg-[#1a1d4f] border border-[#2a2d6f] rounded-xl p-6 w-96">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="font-medium text-lg text-white">R$ {selectedPayment.link ? parseFloat(selectedPayment.link.split('|')[0] || '0').toFixed(2) : '0.00'}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-400">Vencimento</p>
                  <p className="font-medium text-white">{new Date(selectedPayment.dueDate).toLocaleDateString('pt-BR')}</p>
                </div>
