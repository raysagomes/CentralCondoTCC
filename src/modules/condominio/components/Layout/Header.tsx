    ...
    try {
      ...
      if (response.ok) {
        ...
        const unreadCount = data.filter(
          (n: any) => !parsedRead.includes(n.id.toString())
        ).length;
  ...
  const markNotificationAsRead = async (notificationId: string) => {
    try {
      ...
      const unreadCount = notifications.filter(
        (n) => !updatedRead.includes(n.id.toString())
      ).length;
      setNotificationCount(unreadCount);
    } catch (error) {
      ...
      <div className="px-6 py-4">
        <div className="flex justify-end items-center space-x-4">
          ...
          <div className="relative" ref={profileRef}>
            ...
            {showProfileDropdown && (
              <div
                className={`absolute right-0 mt-2 w-48 ${theme.secondaryBg} border ${theme.border} rounded-xl shadow-xl`}
              >
          ...
          <div className="relative" ref={notificationRef}>
            ...
            {showNotifications && (
              <div
                className={`absolute right-0 mt-2 w-80 ${theme.secondaryBg} border ${theme.border} rounded-xl shadow-xl z-50`}
              >
                ...
                <div className="max-h-96 overflow-y-auto">
                  {notifications.length > 0 ? (
                    notifications.slice(0, 5).map((notification) => {
                      const isRead =
                        notification.seen || readNotifications.includes(notification.id.toString());
                      return (
                        <div
                          key={notification.id}
                          onClick={() => {
